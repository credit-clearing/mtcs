...
pub fn validate(op: Op) -> ExternResult<ValidateCallbackResult> {
    match op.to_type::<EntryTypes, LinkTypes>()? {
        ...
        OpType::RegisterCreateLink {
            ...
            match link_type {
                LinkTypes::DebtorToObligations => {
                    validate_create_link_debtor_to_obligations(
                        action,
                        base_address,
                        target_address,
                        tag,
                ...
                LinkTypes::CreatorToObligations => {
                    validate_create_link_creator_to_obligations(
                        action,
                        base_address,
                        target_address,
                        tag,
                ...
                LinkTypes::ObligationUpdates => {
                    validate_create_link_obligation_updates(
                        action,
                        base_address,
                        target_address,
                        tag,
        ...
        OpType::RegisterDeleteLink {
            ...
            match link_type {
                LinkTypes::DebtorToObligations => {
                    validate_delete_link_debtor_to_obligations(
                        action,
                        original_action,
                        base_address,
                        target_address,
                ...
                LinkTypes::CreatorToObligations => {
                    validate_delete_link_creator_to_obligations(
                        action,
                        original_action,
                        base_address,
                        target_address,
                ...
                LinkTypes::ObligationUpdates => {
                    validate_delete_link_obligation_updates(
                        action,
                        original_action,
                        base_address,
                        target_address,
        ...
        OpType::StoreRecord(store_record) => {
            match store_record {
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::Obligation(obligation) => {
                            let result = validate_create_obligation(
                                EntryCreationAction::Update(action.clone()),
                                obligation.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_obligation: Option<Obligation> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_obligation = match original_obligation {
